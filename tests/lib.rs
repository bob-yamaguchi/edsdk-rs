...
mod wrapper_tests{
    ...
    fn wrapper_tests(){
        ...
        for device in &devices{
            let _ = device.as_ref().map(|dev|{
                let device_info = dev.get_device_info();
        ...
        if devices.len() > 0{
            let any = std::mem::replace(&mut devices[0], None);
            if any.is_some(){
                writeln!(&mut std::io::stdout(), "device is some").unwrap();
                devices[0] = any;
            }
        }
        for device in devices{
            device.as_ref().map(|dev|{
                let result = dev.open_session();
                if result.is_ok(){
